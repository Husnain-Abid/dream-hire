const CategoriesSection = () => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 mb-16">
        <h1 className="text-4xl font-normal text-gray-900 mb-12">Browse all categories</h1>
          ...
          {subcategories.map((column, columnIndex) => (
            <div key={columnIndex} className="space-y-4">
              {column.map((subcategory, index) => (
                <a key={index} href="#" className="block text-gray-600 hover:text-blue-600 transition-colors">
                  {subcategory}
                </a>
              ))}
              {columnIndex === subcategories.length - 1 && (
                <a href="#" className="block text-[#5865F2] hover:text-blue-700 transition-colors">
                  View More 28
                </a>
              )}
            </div>
